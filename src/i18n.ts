...
export const UI = {
  ...
  linBriefDiagnose: { en: "Now the real work: the DIAGNOSE phase, which consultants call the 'as-is'. Start on 10 — each domain manager there sets you a check, but no one upstairs opens their door until you've cleared all seven. Pass the last one and come straight back to me for a debrief — only then does the whole Nike C-suite on 15 take your meetings. That's where the evidence is; their teams on 11 fill in the detail. Then you write the diagnosis up yourself and bring it to me as a file. I'll tell you now exactly what I mark it against, so nothing I hand back is a surprise. FOUR THINGS. One: at least THREE distinct pain points, in different domains — one finding is an observation, not a diagnosis, and you'll have sat through seven interviews to find them. Two: every claim traces back to something you were actually told, or to the project files. Three: rank them — which hurts most, and why. Don't hand me a flat list. Four: no solutions yet, just what's broken and how you know. I'll take it claim by claim against your transcripts, hand back anything unsupported with my wording on it, and tell you where you stand every single time you submit — the strongest thing in it, the weakest, and the one thing to do next.", zh: "现在是真正的工作：【诊断】阶段，顾问管它叫「现状（as-is）」。从10层开始——那里每位领域经理都会给你一道考核，但楼上没有人会开门，直到你把七项全部通过。通过最后一项后直接回来找我复盘——之后15层耐克全部高管才会接见你。证据在他们那儿；11层的团队补充细节。访谈完，你自己把诊断写出来，做成文件交给我。我先把评判标准跟你讲清楚，这样我退回来的每一条你都不会觉得意外。四条要求。第一：至少【三个】独立痛点，且分布在不同领域——单一发现只是观察，谈不上诊断，何况你要访谈七个人。第二：每条主张都必须能追溯到你实际听到的内容，或者项目文件。第三：排出优先级——哪一条最要命、为什么。别丢给我一张不分轻重的清单。第四：暂时不谈解决方案，只说清楚哪里出了问题、你怎么知道的。我会逐条拿去比对你的访谈记录，把站不住的附上我改好的措辞还给你；而且你每交一次，我都会告诉你现在什么水平——文档里最扎实的一点、最薄弱的一点，以及下一步该做的那件事。" },
