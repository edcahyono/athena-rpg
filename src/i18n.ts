...
export const UI = {
  ...
  linBriefBenchmark: { en: "Good, the client agreed the diagnosis. Now BENCHMARK: measure Nike against real peers — Anta, Li-Ning, adidas — on numbers they actually disclose. For Greater China that means EBIT margin and revenue growth; net profit, inventory and ROE aren't broken out, so quoting them will get you challenged. Find where we're genuinely weaker and where the opening is, then bring me a prioritised direction. Interview all seven executives before you do — you can't rank priorities across seven functions having heard from four. I convene the meeting here on 12; the CEO and CFO are the ones judging it.", zh: "很好，客户认可了诊断。现在进入【对标】：在对手真正披露的数据上，把耐克和安踏、李宁、阿迪达斯做对比。大中华区能用的是 EBIT 利润率和收入增速；净利润、库存、ROE 并未单独披露，引用会被当场质疑。找出我们真正更弱的地方、以及机会在哪，然后带一个有优先级的方向给我。来之前先把七位高管都谈完——只听了四位，你没法给七个职能排优先级。会议由我在12层召集，评判的是 CEO 和 CFO。" },
