...
export const UI = {
  ...
  linBriefDiagnose: { en: "Now the real work: the DIAGNOSE phase, which consultants call the 'as-is'. Start on 10 — each domain manager there sets you a check, and passing it unlocks their opposite number in the Nike C-suite on 15. That's where the evidence is; their teams on 11 fill in the detail. Then write the diagnosis up yourself — the pain points, the findings, and what each one rests on — and bring it back to me. I'll take it claim by claim against your transcripts and the project files, and hand back anything that isn't supported, with my wording on it. Fix those and come again. No solutions yet: just what's broken, and how you know.", zh: "现在是真正的工作：【诊断】阶段，顾问管它叫「现状（as-is）」。从10层开始——那里每位领域经理都会给你一道考核，通过了就解锁15层耐克高管中对应的那一位。证据在他们那儿；11层的团队补充细节。然后自己把诊断写出来——痛点、发现，以及每一条的依据——再拿回来给我。我会逐条拿去比对你的访谈记录和项目文件，把站不住的还给你，并附上我改好的措辞。改完再来一次。先别谈解决方案：只说清楚哪里出了问题、你怎么知道的。" },
