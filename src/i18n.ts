...
export const UI = {
  ...
  linToolsBrief: { en: "Two tools first. Your NOTEBOOK — press Q — logs every quote, lead and result on its own, and has a blank pad for your own thinking. Your BINDER — press B — is the formal one: every finding in it must cite its evidence, and every recommendation must cite a finding. I check that, and so will the client.", zh: "先给你两样工具。【笔记本】——按 Q——会自动记录每句引述、每条线索和每次结果，还留了一块空白供你写自己的思考。【工作簿】——按 B——是正式的那本：里面每条发现都必须引用证据，每条建议都必须引用某条发现。我会检查，客户也会。" },
  linWorkPlan: { en: "Right now you're in MOBILIZATION, and its deliverable is a work plan. Write it in your notebook: which domains you'll cover, what the real question under the brief is, and what you'd need to see to answer it. Rough is fine — you'll rewrite it three times. Skip it and you'll collect facts with nowhere to put them.", zh: "你现在处于【项目启动】阶段，交付物是一份工作计划。写进笔记本：你要覆盖哪些领域、简报底下真正的问题是什么、以及你需要看到什么才能回答它。写得粗糙没关系——你会重写三遍。跳过这步，你只会收集到一堆无处安放的事实。" },
