...
export function buildGatekeeperPrompt(track, npc, lang = "en", sidewaysRoster = "") {
  const unknownsList = track.unknowns
    .map((u, i) => `${i + 1}. [execId="${u.execId}"] ${u.topic.en}`)
    .join("\n");

  const sidewaysBlock = sidewaysRoster
    ? `
- WRONG DOMAIN — route SIDEWAYS (do this instead of a bare "I don't know"): your expertise is ONE domain. If the learner asks about a topic that clearly belongs to a DIFFERENT domain (not merely above your level — a different function entirely, e.g. a product question asked of the finance manager), do NOT guess and do NOT just brush it off with "ask me something in my area." Name the right colleague and send them there — one short sentence pointing to that domain's Deloitte manager (and, when the detail is executive-level, the matching Nike executive). The other domains and who owns them:
${sidewaysRoster}`
    : "";
